...
function buildSiteStats(
  ...
  const keyEvents = keyEventDefs.map(({ label, types }) => ({
    label,
    count: types.reduce((sum, t) => sum + (eventCounts[t] ?? 0), 0),
  }));
...
export async function GET() {
  ...
  const [sfSessions, sixSessions, sfEvents, sixEvents] = await Promise.all([
    fetchSessions("sf"),
    fetchSessions("six"),
    fetchEvents("sf"),
    fetchEvents("six"),
  ...
  return NextResponse.json({ sites, dailyTrend: buildDailyTrend(sfSessions, sixSessions) });
}
